...
var winTop=0; // This element is to set the top position of the small popup window 
//which is opened when the RichTextEditing option (ie the background colour is clicked)
...
function getOffsetTop(elm) {
	...
	var parents_up = 2; //the positioning div is 2 elements up the tree
		//added for setting the position of popup window
		mOffsetTop =winTop;
	while(parents_up > 0) {
...
//Customized for ASM module
function writeRichTextASM(rte, html, width, height, buttons, readOnly) {
	if (isRichText) {
		if (allRTEs.length > 0) allRTEs += ";";
		allRTEs += rte;
